...
def workflow(inchi, identity, file):
    try:
        ...
        with open("template/candidates.fasta", 'w') as f:
        ...
        test_X_ = []
        for ind in df_transporters.index:
            emb = df_transporters["ESM1b"][ind]
            test_X.append(emb)
        ...
        # run blastp
        blastp_command = f'blastp -query template/candidates.fasta -db ../ICT-DB/blastdb/ictdb -outfmt 6 -out -'
        ...
        if result.returncode == 0:
            ...
            df_transporter.to_csv(f"TransDW_Result.csv", index=False)
